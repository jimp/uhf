...
#        end
        a=Asset.create!(:uploaded_data=>params[:NewFile])
        @errorNumber = 0
        js = "if(window.parent.frames[\'frmUpload\']){
            window.parent.frames[\'frmUpload\'].OnUploadCompleted(#{@errorNumber});
          }else{
            window.parent.OnUploadCompleted(#{@errorNumber},'#{a.public_filename}', '#{a.public_filename}', '');
          }"
        render :text => %Q'<script type="text/javascript">#{js}</script>'
#      end
    rescue => e
      @errorNumber = 110 if @errorNumber.nil?
      raise e
      #render :text => %Q'<script>window.parent.frames[\'frmUpload\'].OnUploadCompleted(#{@errorNumber});</script>'
      render :text => %Q'<script>window.parent.OnUploadCompleted(#{@errorNumber});</script>'
    end
  end
