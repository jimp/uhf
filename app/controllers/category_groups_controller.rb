class CategoryGroupsController < ApplicationController
  ...
  def index
    @category_groups = CategoryGroup.find(:all)
  ...
  def create
    ...
    respond_to do |format|
      if @category_group.save
        flash[:notice] = 'CategoryGroup was successfully created.'
        format.html { redirect_to(category_groups_path) }
  ...
  def update
    ...
    respond_to do |format|
      if @category_group.update_attributes(params[:category_group])
        flash[:notice] = 'CategoryGroup was successfully updated.'
        format.html { redirect_to(category_groups_path) }
        format.xml  { head :ok }
      else
